...
(function (Phaser) {
    var FontTexture = (function (_super) {
        ...
        FontTexture.prototype.loadFrameData = function (data) {
            ...
            this.letterSpacing = (this.atlasData).meta.kerning;
            this.spaceWidth = (this.atlasData).meta.spaceWidth;
            this.atlasCache = {
            };
            this.widthMap = {
            };
            var total = this.atlasData["frames"].length;
            console.log("parse frame data", this.atlasData, total);
        };
        ...
        FontTexture.prototype.draw = function (targetX, targetY, spriteID) {
            ...
            this._game.stage.context.drawImage(this._texture, rect.x, rect.y, rect.width, rect.height, targetX, targetY, rect.width, rect.height);
        };
        return FontTexture;
    })(Phaser.GameObject);
...
(function () {
    ...
    function init() {
        ...
        myGame.loader.addImageFile('nokia-24', 'assets/textures/nokia-24-white-shadow.png');
        myGame.loader.addTextFile('nokia-24-atlas', 'assets/textures/nokia-24-white-shadow.txt');
        myGame.loader.load();
    }
    function create() {
        for(var i = 0; i < totalButtons; i++) {
            var button = myGame.createSprite(0, 0, "entities");
            button.exists = false;
            buttons.push(button);
        }
    ...
    function click(value) {
        console.log("click", myGame.input.x, myGame.input.y);
        if(mode == START_GAME) {
    ...
    function newRound() {
        ...
        for(var i = 0; i < totalButtons; i++) {
            var value = myGame.rnd.integerInRange(1, 11);
            var button = buttons[i];
        ...
        mainButton.frameName = "large-font-" + (mainNumber < 10 ? "0" + mainNumber.toString() : mainNumber.toString()) + ".png";
    ...
    function render() {
        smallFont.drawText("score: " + score.toString(), 0, 0, Phaser.ALIGN.LEFT);
    }
    function checkHand() {
        if(mainNumber > 21) {
            console.log("bust");
    ...
    function submitScore() {
        if(mainNumber > 15) {
            var tmpScore = mainNumber * (totalButtons - cardsUsed) - valueLost * ((mainNumber == 21) ? 2 : 1);
            score += tmpScore;
