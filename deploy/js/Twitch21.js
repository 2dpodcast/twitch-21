...
(function () {
    ...
    function increateMainNumber(newValue, speed, delay) {
        if (typeof speed === "undefined") { speed = 500; }
        if (typeof delay === "undefined") { delay = 0; }
        scoreTween.to({
            value: newValue
        }, speed, Phaser.Easing.Quadratic.Out);
    ...
    function newRound() {
        ...
        var tmpValue = Math.round(tweenedValues.value);
    ...
    function getFrameName(tmpValue) {
        var frameName = "larg-font-" + (tmpValue < 10 ? "0" + tmpValue.toString() : tmpValue.toString());
        if(mode == GAME_OVER) {
            frameName += "-green";
        }
    ...
    function submitScore() {
        if(mainNumber >= 15) {
            var tmpScore = mainNumber * (totalButtons + 1 - cardsUsed) - valueLost * ((mainNumber == 21) ? 2 : 1);
            score += tmpScore;
